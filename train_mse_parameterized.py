...
def main(args, cfg):
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, label) in enumerate(train_loader):
            ...
            with torch.no_grad():
                emb_teacher = F.normalize(backbone_teacher(img), dim=1)
            emb_student = F.normalize(backbone_student(img), dim=1)

            loss_v = criterion(emb_student, emb_teacher)
            loss_v.backward()
...
if __name__ == "__main__":
    ...
    print(
        f"mse | dataset: {args_.dataset} | teacher: {args_.teacher} | student: {args_.student}"
    )

    args_.experiment_id = (
        f"{args_.student}_{args_.dataset}_msekd_teacher={args_.teacher}"
    )
